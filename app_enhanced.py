...
def extract_amount_with_validation(tags: List[Dict], params: Dict) -> Tuple[float, float]:
    ...
    # 金額正則（支援負數與小數）
    price_pattern = r'(-?\d+[' + re.escape(t_sep + d_sep) + r']\d{2})'
    ...
    for t in total_rows:
        ...
        for p in prices:
            val = float(p.replace(t_sep, '').replace(d_sep, '.'))
            if val > 0:  # 排除負數
...
def main():
    st.set_page_config(page_title="考察支出登錄系統 [準確度增強版]", layout="wide")
    init_session()
