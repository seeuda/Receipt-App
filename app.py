...
def calculate_salted_uid(file_content: bytes, user_name: str) -> str:
    """
    計算加鹽後的唯一識別碼 (Salted UID)。
    結合檔案指紋與使用者姓名，確保同一張照片由同一人上傳時可觸發『覆蓋更新』邏輯。
    """
...
def add_project_to_registry(name: str, sheet_id: str) -> bool:
    ...
    try:
        ...
        new_row = [""] * len(headers)
        new_row[idx_name], new_row[idx_id], new_row[idx_status] = name, sheet_id, "TRUE"
...
def load_project_users(tid: str) -> List[str]:
    """載入專案專屬人員名單 """
    try:
        gc = get_gspread_client(); sh = gc.open_by_key(tid)
...
def load_all_configs() -> Dict:
    """載入 40 國在地化配置檔 """
    configs = {}
    emoji_map = {"tw": "🇹🇼", "jp": "🇯🇵", "de": "🇩🇪", "us": "🇺🇸", "gb": "🇬🇧"} # (示意，完整版包含 40 國)
    for f in glob.glob("configs/*.json"):
        iso = os.path.basename(f).split('_')[0].lower()
        with open(f, 'r', encoding='utf-8') as j:
            d = json.load(j)
            label = f"{emoji_map.get(iso, '🌐')} {d.get('country', iso)}"
            configs[label] = d
    return configs
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("🆕 註冊新專案"):
            n_p = st.text_input("專案名稱"); i_p = st.text_input("試算表 ID")
            if st.button("確認註冊") and n_p and i_p:
                if add_project_to_registry(n_p, i_p): st.success("成功！請刷新"); st.rerun()
            ...
            try:
                ...
                for f in files:
                    ...
                    st.session_state['data'].append({
                        "商店名稱": v, "參考品項": it, "消費日期": d, 
                        "外幣金額": a, "匯率": a_r, "備註": "", "UID": salted_uid
                    })
                st.rerun()
            except Exception as e: st.error(f"辨識異常: {e}")

    if st.session_state['data']:
        st.info("💡 您可以直接在下方表格修正辨識錯誤，同步時將以修正後的內容覆蓋雲端舊資料。")
        ...
        if st.button("📤 同步至雲端", type="primary", use_container_width=True):
            ...
            st.success(f"✅ 同步完成：新增 {sc} 筆，修正覆蓋 {uc} 筆。")
